...
No* carregarRecursivo(FILE *f) {
    ...
    //cria o n� com o texto (pula os 3 primeiros caracteres "[X]")
    No *novo = criarNo(linha + 3);

    //se a linha come�ar com [P], � uma pergunta, ent�o tem filhos
    if (linha[1] == 'P') {
        novo->sim = carregarRecursivo(f);
        novo->nao = carregarRecursivo(f);
    }
    //se for [A], � animal, n�o faz nada (filhos continuam NULL)
...
No* carregarBancoDeDados() {
    ...
    //se o arquivo n�o existir (primeira vez), cria o padr�o
    if (raiz == NULL) {
...
//l�gica do Jogo (Igual a anterior)
